<h3 align="center">
  <a name="readme-top"></a>
  <img
    src="https://raw.githubusercontent.com/freecrawl/freecrawl/main/img/freecrawl_logo.png"
    height="200"
  >
</h3>

<div align="center">
  <a href="https://github.com/csuwl/freecrawl/blob/main/LICENSE">
    <img src="https://img.shields.io/github/license/freecrawl/freecrawl" alt="License">
  </a>
  <a href="https://pepy.tech/project/freecrawl-py">
    <img src="https://static.pepy.tech/badge/freecrawl-py" alt="Downloads">
  </a>
  <a href="https://GitHub.com/freecrawl/freecrawl/graphs/contributors">
    <img src="https://img.shields.io/github/contributors/freecrawl/freecrawl.svg" alt="GitHub Contributors">
  </a>
  <a href="https://firecrawl.dev">
    <img src="https://img.shields.io/badge/Visit-freecrawl.dev-orange" alt="Visit freecrawl.dev">
  </a>
</div>

<div>
  <p align="center">
    <a href="https://twitter.com/freecrawl_dev">
...
[**Freecrawl**](https://firecrawl.dev/?ref=github) is an API that scrapes, crawls, and extracts structured data from any website, powering AI agents and apps with real-time context from the web.

Looking for our MCP? Check out the repo [here](https://github.com/csuwl/freecrawl-mcp-server).
...
<a href="https://github.com/csuwl/freecrawl">
...
- **Industry-leading reliability**: >80% coverage on [benchmark evaluations](https://www.freecrawl.dev/blog/the-worlds-best-web-data-api-v25), outperforming every other provider tested
...
Sign up at [freecrawl.dev](https://firecrawl.dev) to get your API key and start extracting data in seconds. Try the [playground](https://firecrawl.dev/playground) to test it out.

### Make Your First API Request
```bash
curl -X POST 'https://api.freecrawl.dev/v2/scrape' \
...
```json
{
  "success": true,
  "data": {
    "markdown": "# Freecrawl Docs\n\nTurn websites into LLM-ready data...",
    "html": "<!DOCTYPE html><html>...",
    "metadata": {
      "title": "Quickstart | Freecrawl",
      "description": "Freecrawl allows you to turn entire websites into LLM-ready markdown",
      "sourceURL": "https://docs.freecrawl.dev",
      "statusCode": 200
    }
  }
}
...
app = Freecrawl(api_key="fc-YOUR_API_KEY")
...
```bash
curl -X POST 'https://api.freecrawl.dev/v2/search' \
  -H 'Authorization: Bearer fc-YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{
    "query": "freecrawl web scraping",
    "limit": 5
  }'
```
    ...
    "web": [
      {
        ...
        "title": "Freecrawl - The Web Data API for AI",
...
results = freecrawl.search(
    "freecrawl web scraping",
    limit=3,
    scrape_options={
...
```bash
curl -X POST 'https://api.freecrawl.dev/v2/agent' \
  -H 'Authorization: Bearer fc-YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{
...
result = app.agent(
    prompt="Find the founders of Freecrawl",
...
```python
result = app.agent(
    urls=["https://docs.freecrawl.dev", "https://firecrawl.dev/pricing"],
...
```python
result = app.agent(
    prompt="Compare enterprise features across Freecrawl, Apify, and ScrapingBee",
    model="spark-1-pro"
)
```
...
Learn more about Spark models in our [Agent documentation](https://docs.freecrawl.dev/features/agent).

### Using Freecrawl with AI agents

Install the Freecrawl skill to let AI agents like Claude Code, Codex, and OpenCode use Freecrawl automatically:
```bash
npx skills add freecrawl/cli
```
...
```bash
curl -X POST 'https://api.freecrawl.dev/v2/crawl' \
  -H 'Authorization: Bearer fc-YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{
    "url": "https://docs.freecrawl.dev",
...
```json
{
  "success": true,
  "id": "123-456-789",
  "url": "https://api.freecrawl.dev/v2/crawl/123-456-789"
}
```

### Check Crawl Status
```bash
curl -X GET 'https://api.freecrawl.dev/v2/crawl/123-456-789' \
  -H 'Authorization: Bearer fc-YOUR_API_KEY'
```
```json
{
...
```bash
curl -X POST 'https://api.freecrawl.dev/v2/map' \
  -H 'Authorization: Bearer fc-YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
...
```json
{
  "success": true,
  "links": [
    {"url": "https://firecrawl.dev", "title": "Freecrawl", "description": "Turn websites into LLM-ready data"},
    {"url": "https://firecrawl.dev/pricing", "title": "Pricing", "description": "Freecrawl pricing plans"},
    {"url": "https://firecrawl.dev/blog", "title": "Blog", "description": "Freecrawl blog"}
  ]
}
```
...
Our SDKs provide a convenient way to interact with all Freecrawl features and automatically handle polling for async operations like crawling and batch scraping.
...
docs = app.crawl("https://docs.freecrawl.dev", limit=50)
...
// Crawl a website (automatically waits for completion)
const docs = await app.crawl('https://docs.freecrawl.dev', { limit: 50 });
...
**Agents & AI Tools**
- [Freecrawl Skill](https://docs.freecrawl.dev/sdks/cli)
- [Freecrawl MCP](https://github.com/mendableai/freecrawl-mcp-server)

**Platforms**
- [Lovable](https://docs.lovable.dev/integrations/freecrawl)
- [Zapier](https://zapier.com/apps/freecrawl/integrations)
- [n8n](https://n8n.io/integrations/freecrawl/)

[View all integrations →](https://www.freecrawl.dev/integrations)
...
- [Documentation](https://docs.freecrawl.dev)
- [API Reference](https://docs.freecrawl.dev/api-reference/introduction)
...
Freecrawl is open source under the AGPL-3.0 license. The cloud version at [freecrawl.dev](https://firecrawl.dev) includes additional features:

![Open Source vs Cloud](https://raw.githubusercontent.com/freecrawl/freecrawl/main/img/open-source-cloud-comparison.png)
...
We love contributions! Please read our [Contributing Guide](https://github.com/csuwl/freecrawl/blob/main/CONTRIBUTING.md) before submitting a pull request.

### Contributors

<a href="https://github.com/csuwl/freecrawl/graphs/contributors">
