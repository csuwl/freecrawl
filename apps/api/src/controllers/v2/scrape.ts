...
import { sendUsageCallbackByApiKey } from "../../services/usage-callback";
...
export async function scrapeController(
  ...
  return withSpan(
    ...
    async span => {
      ...
      // 发送用量回调到 Dashboard (通过 API Key)
      try {
        const apiKey = req.headers["authorization"]?.replace("Bearer ", "");
        await sendUsageCallbackByApiKey(
          apiKey,
          "scrape",
          Number(doc?.metadata?.creditsUsed) || 1,
          1
        );
      } catch (e) {
        logger.warn("Usage callback failed", { error: e });
      }
