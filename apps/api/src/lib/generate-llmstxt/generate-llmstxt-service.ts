  ...
  try {
    ...
    for (let i = 0; i < urls.length; i += 10) {
      ...
      for (const result of batchResults) {
        ...
        llmsFulltxt += `<|freecrawl-page-${i + batchResults.indexOf(result) + 1}-lllmstxt|>\n## ${result.title}\n${result.markdown}\n\n`;
      }
