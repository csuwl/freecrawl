...
export function getErrorContactMessage(errorId?: string): string {
  if (isSelfHosted()) {
    ...
    return errorId
      ? `An unexpected error occurred. Please contact help@freecrawl.com for help. Your exception ID is ${errorId}`
      : "An unexpected error occurred. Please contact help@freecrawl.com for help.";
  }
}
