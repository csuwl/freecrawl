...
description: "Freecrawl's enhanced Change Tracking feature now provides detailed insights into webpage updates, including diffs and structured data comparisons."
pubDate: "Apr 14, 2025"
heroImage: "../../assets/blog/changeTracking.jpg"
category: "updates"
---

**Welcome to Launch Week III, Day 1! Today we’re excited to announce Change Tracking** — an enhanced Freecrawl feature that automatically detects and details changes on websites, now available in beta for all users.
...
Freecrawl’s Change Tracking feature integrates effortlessly into your existing workflows with two simple request methods—scrape and crawl. You must specify the `markdown` format in addition to `changeTracking`:

### Scrape Request Example:

```typescript
const scrapeResponse = await app.scrapeUrl("https://freecrawl.dev", {
...
```typescript
const crawlResponse = await app.crawlUrl("https://freecrawl.dev", {
...
When enabled, Freecrawl compares current scrapes against previous versions based on URL, team ID, and markdown format:
...
- **Learn more:** [Read the docs for `/scrape`](https://docs.freecrawl.dev/features/change-tracking) and [the docs for `/crawl`](https://docs.freecrawl.dev/features/crawl#change-tracking).
- **Get help:** Join our [Discord community](https://discord.gg/S7Enyh9Abh) or contact [help@freecrawl.com](mailto:help@freecrawl.com).

**Ready to track detailed content changes?** [Sign up for Freecrawl](https://freecrawl.dev/signup) and start today.
