...
async function main() {
  ...
  const freecrawl = new Freecrawl({ apiKey });
  ...
  const crawlAll = await freecrawl.crawl("https://docs.freecrawl.dev", { limit: 5 });
  ...
  const crawlStart = await freecrawl.startCrawl("https://docs.freecrawl.dev", { limit: 5 });
  ...
  const crawlSingle = await freecrawl.getCrawlStatus(crawlJobId, { autoPaginate: false });
  ...
  const crawlLimited = await freecrawl.getCrawlStatus(crawlJobId, {
    autoPaginate: true,
  ...
  const batchAll = await freecrawl.batchScrape([
  ...
  // Use this when you need to control how many pages to fetch or want to process results incrementally
  const batchStart = await freecrawl.startBatchScrape([
    "https://docs.freecrawl.dev",
    "https://freecrawl.dev",
  ], { options: { formats: ["markdown"] } });
  ...
  const batchSingle = await freecrawl.getBatchScrapeStatus(batchJobId, { autoPaginate: false });
  ...
  const batchLimited = await freecrawl.getBatchScrapeStatus(batchJobId, {
    autoPaginate: true,
    maxPages: 2,        // Only fetch 2 pages maximum
