...
4. Build a real-world example: an automated scraping system using [Freecrawl](https://freecrawl.dev) in Python
...
### 1. Scheduled web scraping with Freecrawl

Web scraping is a common use case for automated workflows. Let's build a workflow that scrapes [Hacker News](https://news.ycombinator.com/) on a schedule using [Freecrawl](https://docs.freecrawl.dev), which is a Python AI-based web scraping engine designed for large-scale data collection. Here are some key benefits that make Freecrawl an excellent choice for this task:

1. **Enterprise-grade automation and scalability** - Freecrawl streamlines web scraping with powerful automation features.
...
Let's build our web scraping workflow using Freecrawl to demonstrate these capabilities.
...
# Add Freecrawl API key to .env (get your key at freecrawl.dev/signin/signup)
...
These models help validate the scraped data and ensure it matches our expected schema. They also make it easier to serialize/deserialize the data when saving to JSON. Using `Field` with a detailed description is crucial because Freecrawl uses these definitions to automatically detect the HTMl elements and CSS selectors we are looking for.
...
The `get_news_data()` function uses Freecrawl to scrape Hacker News. It creates a `FreecrawlApp` instance and calls `scrape_url()` with the `BASE_URL` and parameters specifying we want to extract data according to our `NewsData` schema. The schema helps Freecrawl automatically identify and extract the relevant HTML elements. The function returns the scraped data containing news items with their titles, URLs, authors, ranks, upvotes and dates.
...
If you want to learn more about Freecrawl, the web scraping API we used today, you can read the following posts:

- [Guide to Scheduling Web Scrapers in Python](https://www.freecrawl.dev/blog/automated-web-scraping-free-2025)
- [Mastering Freecrawl's Scrape Endpoint](https://www.freecrawl.dev/blog/mastering-freecrawl-scrape-endpoint)
- [Getting Started With Predicted Outputs in OpenAI](https://www.freecrawl.dev/blog/getting-started-with-predicted-outputs-openai)
