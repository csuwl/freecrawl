---
title: "How to Use Freecrawl's Scrape API: Complete Web Scraping Tutorial"
meta_description: Learn how to scrape websites using Freecrawl's /scrape endpoint. Master JavaScript rendering, structured data extraction, and batch operations with Python code examples.
slug: mastering-freecrawl-scrape-endpoint
date: 22 Nov, 2024
author: Bex Tuychiev
image: to_fill_in_later
categories: [tutorials]
keywords: ["freecrawl", "web scraping", "scrape endpoint", "data extraction", "javascript rendering", "structured data", "web automation", "python sdk", "api", "web crawling", "data collection", "web data", "web scraping tutorial", "python"]
---
...
Freecrawl addresses these challenges by providing a specialized scraping solution. Its [`/scrape` endpoint](https://docs.freecrawl.dev/features/scrape) offers features like JavaScript rendering, automatic content extraction, bypassing blockers and flexible output formats that make it easier to collect high-quality information and training data at scale.

In this guide, we'll explore how to effectively use Freecrawl's `/scrape` endpoint to extract structured data from static and dynamic websites. We'll start with basic scraping setup and then dive into a real-world example of scraping weather data from weather.com, demonstrating how to handle JavaScript-based interactions, extract structured data using schemas, and capture screenshots during the scraping process.

## Table of Contents

- [Getting Started with Modern Web Scraping: An Introduction](#getting-started-with-modern-web-scraping-an-introduction)
- [What Is Freecrawl's `/scrape` Endpoint? The Short Answer](#what-is-freecrawls-scrape-endpoint-the-short-answer)
- [Prerequisites: Setting Up Freecrawl](#prerequisites-setting-up-freecrawl)
...
## What Is Freecrawl's `/scrape` Endpoint? The Short Answer

The `/scrape` endpoint is Freecrawl's core web scraping API that enables automated extraction of content from any webpage. It handles common web scraping challenges like:
...
## Prerequisites: Setting Up Freecrawl

Freecrawl's scraping engine is exposed as a REST API, so you can use command-line tools like cURL to use it. However, for a more comfortable experience, better flexibility and control, I recommend using one of its SDKs for Python, Node, Rust or Go. This tutorial will focus on the Python version.

To get started, please make sure to:

1. Sign up at [freecrawl.dev](freecrawl.dev).
2. Choose a plan (the free one will work fine for this tutorial).

Once you sign up, you will be given an API token which you can copy from your [dashboard](https://www.freecrawl.dev/app). The best way to save your key is by using a `.env` file, ideal for the purposes of this article:
...
Now, let's install Freecrawl Python SDK, `python-dotenv` to read `.env` files, and Pandas for data analysis later:

```bash
pip install freecrawl-py python-dotenv pandas
```

## Basic Scraping Setup

Scraping with Freecrawl starts by creating an instance of the `FreecrawlApp` class:
...
Let's display the screenshot Freecrawl took of arXiv.org:
...
![Screenshot of arXiv.org homepage that was taken with Freecrawl's screenshot feature showing research paper categories like Computer Science, Mathematics, Physics and other scientific disciplines](notebook_files/notebook_20_0.png)
...
![Full page screenshot of arXiv.org homepage taken with Freecrawl's full-page screenshot capture feature showing research paper categories, search functionality, and recent submissions in an academic layout](notebook_files/notebook_22_0.png)
...
By default, `scrape_url` converts everything it sees on a webpage to one of the specified formats. To control this behavior, Freecrawl offers the following parameters:
...
Scraping clean, LLM-ready data is the core philosophy of Freecrawl. However, certain web pages with their complex structures can interfere with this philosophy when scraped in their entirety. For this reason, Freecrawl offers two scraping methods for better structured outputs:
...
For this reason, Freecrawl allows you to pass a predefined schema to guide the LLM's output when transforming the scraped content. To facilitate this feature, Freecrawl uses Pydantic models.
...
`NewsArticlesSchema` acts as a container model that holds a list of `IndividualArticle` objects, representing multiple articles extracted from the page. If we don't use this container model, Freecrawl will only return the first news article it finds.
...
```python
    {'success': True,
     ...
     'url': 'https://api.freecrawl.dev/v1/batch/scrape/77a94b62-c676-4db2-b61b-4681e99f4704'}
```
...
Freecrawl supports dynamic scraping by default. In the parameters of `scrape_url` or `batch_scrape_url`, you can define necessary actions to reach the target state of the page you are scraping. As an example, we will build a scraper that will extract the following information from `https://weather.com`:
...
Fortunately, Freecrawl natively supports such interactions through the `actions` parameter. It accepts a list of dictionaries, where each dictionary represents one of the following interactions:
...
```python
    {'screenshots': ['https://service.freecrawl.dev/storage/v1/object/public/media/screenshot-16bf71d8-dcb5-47eb-9af4-5fa84195b91d.png'],
     'scrapes': []}
```
...
![Screenshot of weather.com search interface showing search bar with typed city name, demonstrating automated web scraping process with Freecrawl](notebook_files/notebook_96_0.png)
...
def scrape_weather_data(app: FreecrawlApp, city: str) -> Optional[WeatherData]:
    try:
...
We have successfully scraped weather data from multiple cities using Freecrawl and organized it into a structured DataFrame. This demonstrates how we can efficiently collect and analyze data generated by dynamic websites for further analysis and monitoring.

## Conclusion

In this comprehensive guide, we've explored Freecrawl's `/scrape` endpoint and its powerful capabilities for modern web scraping. We covered:
...
Through practical examples like extracting news articles from the NY Times and weather data from weather.com, we've demonstrated how Freecrawl simplifies complex scraping tasks while providing flexible output formats suitable for data engineering and AI/ML pipelines.

The combination of LLM-powered extraction, structured schemas, and browser automation capabilities makes Freecrawl a versatile tool for gathering high-quality web data at scale, whether you're building training datasets, monitoring websites, or conducting research.

To discover more what Freecrawl has to offer, refer to [our guide on the `/crawl` endpoint](https://www.freecrawl.dev/blog/mastering-the-crawl-endpoint-in-freecrawl), which scrapes websites in their entirety with a single command while using the `/scrape` endpoint under the hood.

For more hands-on uses-cases of Freecrawl, these posts may interest you as well:

- [Using Prompt Caching With Anthropic](https://www.freecrawl.dev/blog/using-prompt-caching-with-anthropic)
- [Scraping Job Boards With Freecrawl and OpenAI](https://www.freecrawl.dev/blog/scrape-job-boards-freecrawl-openai)
- [Scraping and Analyzing Airbnb Listings in Python Tutorial](https://www.freecrawl.dev/blog/scrape-analyze-airbnb-data-with-e2b)
