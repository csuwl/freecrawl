---
title: "How to Generate a Sitemap Using Freecrawl's /map Endpoint: A Complete Guide"
meta_description: "Learn how to generate XML and visual sitemaps using Freecrawl's /map endpoint. Step-by-step guide with Python code examples, performance comparisons, and interactive visualization techniques for effective website mapping."
slug: how-to-generate-sitemap-using-freecrawl-map-endpoint
date: 23 Nov, 2024
author: Bex Tuychiev
image: to_fill_in_later
categories: [tutorials]
keywords: [sitemap generator, website mapping, freecrawl, url discovery, site structure analysis, seo optimization, web crawling, website indexing, automated sitemap, site mapping tools, website architecture, sitemap creation, web development tools, website maintenance, seo tools, map endpoint, freecrawl map endpoint, python sitemap generator, visual sitemap creator, website structure visualization, interactive site mapping, xml sitemap generation, website crawling api, sitemap automation tools, website structure analysis tools]
---

## Introduction

In this guide, we'll explore Freecrawl's `/map` endpoint - a powerful tool for automated website mapping and URL discovery. We'll cover what it does, why it matters, and how to use it effectively in your web development workflow.
...
- [Understanding Freecrawl's `/map` Endpoint: Features and Benefits](#understanding-freecrawls-map-endpoint-features-and-benefits)
  ...
  - [How Freecrawl's `/map` Endpoint Solves These Challenges](#how-freecrawls-map-endpoint-solves-these-challenges)
...
## Understanding Freecrawl's `/map` Endpoint: Features and Benefits
...
### How Freecrawl's `/map` Endpoint Solves These Challenges
...
Freecrawl is a scraping engine exposed as a REST API, which means you can use it from the command-line using cURL or by using one of its SDKs in Python, Node, Go or Rust. In this tutorial, we will use its Python SDK, so please install it in your environment:

```bash
pip install freecrawl-py
```

The next step is obtaining a Freecrawl API key by signing up at [freecrawl.dev](freecrawl.dev) and choosing a plan (the free plan is fine for this tutorial).
...
response = app.map_url(url="https://freecrawl.dev")
```

In this code snippet, we're using the Freecrawl Python SDK to map a URL. Let's break down what's happening:

First, we import two key components:

- FreecrawlApp from the freecrawl package, which provides the main interface to interact with Freecrawl's API
- `load_dotenv` from `dotenv` to load our environment variables containing the API key

After importing, we initialize a FreecrawlApp instance, which automatically picks up our API key from the environment variables.

Finally, we make a request to map the URL `https://freecrawl.dev` using the`map_url()` method. This crawls the website and returns information about its structure and pages, taking about two seconds on my machine (the speed may vary based on internet speeds).
...
If you read our [separate guide on the `/crawl` endpoint](https://www.freecrawl.dev/blog/mastering-the-crawl-endpoint-in-freecrawl) of Freecrawl, you may notice one similarity between it and the `/map` endpoint:
...
![Comparison chart showing significant performance difference between map and crawl endpoints of Freecrawl with map being much faster](notebook_files/notebook_42_0.png)
...
![XML sitemap example showing structured website data with URLs, last modified dates, change frequencies and priorities, generated using Freecrawl](notebook_files/image.png)
...
![Interactive Sankey diagram visualization showing hierarchical structure of Stripe documentation with color-coded sections and flow widths representing page counts, generated using Freecrawl's /map endpoint](images/stripe_docs.png)
...
![Interactive Sankey diagram showing hierarchical website structure of PyData.org with color-coded sections, flow visualization and navigation paths for improved site architecture understanding, generated using Freecrawl's map endpoint and Plotly](images/pydata.png)
...
To discover what more Freecrawl has to offer, be sure to read the following related resources:

- [Freecrawl Documentation](docs.freecrawl.dev)
- [Freecrawl Blog](https://www.freecrawl.dev/blog/category/tutorials)
- [Freecrawl API Reference](https://docs.freecrawl.dev/api-reference/introduction)

## Frequently Asked Questions

### How fast is Freecrawl's `/map` endpoint?
